...
export const InitialModal = () => {
  ...
  const onSubmit = useCallback(async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post("/api/servers", values); // Wysyłanie danych na serwer
      form.reset(); // Resetowanie formularza po sukcesie
      router.refresh(); // Odświeżenie strony
      window.location.reload(); // Przeładowanie strony
    } catch (error) {
      console.error("Error submitting form:", error); // Obsługa błędów
    }
  }, [form, router]);
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <FileUpload
                          endpoint="imageUploader" // Przekazujemy endpoint
                          value={field.value}
